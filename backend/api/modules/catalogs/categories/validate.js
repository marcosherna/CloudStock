const { param, checkSchema } = require("express-validator");

const categorySchema = {
  id: {
    in: ["body"],
    exists: {
      errorMessage: "ID is required",
    },
    isInt: {
      errorMessage: "ID must be a positive integer",
    },
    toInt: true,
  },
  name: {
    in: ["body"],
    exists: {
      errorMessage: "Name is required",
    },
    isString: {
      errorMessage: "Name must be a string",
    },
    trim: true,
    escape: true,
  },
  description: {
    in: ["body"],
    optional: true,
    isString: {
      errorMessage: "Description must be a string",
    },
    trim: true,
    escape: true,
  },
  status: {
    in: ["body"],
    optional: true,
    isBoolean: {
      errorMessage: "Description must be a boolean",
    },
    toBoolean: true,
    isIn: {
      options: [[true, false]],
      errorMessage: "Status must be true or false",
    },
  },
};

const { id, ...categorySchemaWithoutId } = categorySchema;
const createSchema = { ...categorySchemaWithoutId };

const { exists, ...fleldsWithoudExists } = categorySchema.name;
const updateSchema = {
  ...categorySchema,
  name: {
    ...fleldsWithoudExists,
    optional: true,
  },
};

module.exports = {
  Id: () =>
    param("id")
      .exists()
      .withMessage("ID is required")
      .isInt({ min: 1 })
      .withMessage("ID must be a positive integer greater than 0")
      .toInt(),
  CreateCategory: () => checkSchema({ ...createSchema }),
  UpdateCategory: () => checkSchema({ ...updateSchema }),
  categorySchema,
  createSchema,
  updateSchema,
};
