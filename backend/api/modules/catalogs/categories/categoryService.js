...
module.exports = function CategoryService({ categoryRepository }) {
    ...
    service.update = async (categoryId, categoryData) => {
        ...
        if (!category) throw new NotFound('Category not found')
        if (categoryData.id != categoryId) throw new Conflict('Category ID mismatch')
 
        const updatedCategory = await categoryRepository
            .update(categoryData,{ where: { id: categoryId } }) 
    ...
    service.delete = async (categoryId) => {
        ...
        const deletedCategory = await categoryRepository.destroy({ where: { id: categoryId } }) 
